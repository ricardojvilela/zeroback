...
IndexNow support was added using the existing public key file, sitemap-only URL validation and a dry-run mode. Six new verified partner prospects were staged from official business websites: five ecommerce product-photography studios and one Portuguese Shopify agency. Each partner receives an individual campaign identifier, a concise two-image test request and an explicit opt-out. The message does not promise commission or position BatchCutout as a replacement for professional photography or retouching.
