...
The real 18.4-second workflow is published on the separate NexaFlow Labs YouTube channel at `https://youtu.be/u-F4J-idMkY`, with tracked links to the free test and proof page. The NexaFlow Labs channel currently uses `@nexaflowlabs-tools` because YouTube retains the previous `@nexaflowlabs` handle for 14 days. YouTube requires phone verification for clickable external description links and a custom thumbnail, so a real automatic frame is used. The earlier copy on the personal channel was made private after the business-channel publication was verified. A NexaFlow Labs LinkedIn company page was not available, and Reddit blocked access behind a CAPTCHA before a business account could be verified.
