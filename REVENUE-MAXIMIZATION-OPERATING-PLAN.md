...
- Internal interface labels such as `result_ready_sticky`, `result_ready_inline`, `account_panel` and `checkout_plan` are excluded from acquisition-source attribution, so admin revenue/lead reporting falls back to the real first/last traffic source.
