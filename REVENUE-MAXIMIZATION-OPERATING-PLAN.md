...
The recommended live action is to pause `[product photo background remover]` in `Search - BatchCutout - Pro Launch` and add the exact-match negative candidates in `google-ads-imports/batchcutout-pro-launch-negative-candidates-2026-07-27.csv`. Exact matching preserves all queries containing `bulk`, `batch`, `multiple` or `mass`, including the query that converted. The candidate terms accounted for 17.30 EUR of identifiable spend over the latest 14 complete days; this amount overlaps the keyword spend and must not be added to it when estimating impact.

This proposal would redirect the existing 8 EUR/day Pro Launch budget toward explicit batch intent rather than reduce or increase the budget. It has not been applied because live keyword and negative-keyword changes require explicit confirmation. The new paid direct-upload handoff remains the only live experiment launched on 2026-07-27.
