...
The Smol Launch company account `nexaflow_labs` was created with `support@batchcutout.com` and email-confirmed. BatchCutout was submitted with its logo, the three validated commercial screenshots, the prepared description, Freemium pricing, and the `SaaS & Tools`, `E-commerce` and `Design Tools` tags. The public product page is `https://smollaunch.com/products/batchcutout`, currently marked `Pending Review`, and its website link preserves `utm_source=smol_launch`, `utm_medium=directory` and `utm_campaign=directory_wave2` before Smol Launch adds its own referral parameter.

The selected Smol Launch option is the zero-cost Basic Free queue without a site badge, using a standard nofollow link. It had 591 products ahead and assigned the week of 2027-09-20, with the platform stating that a human usually reviews submissions within one day and approved launches go live on Monday at 09:00 PT. Paid Premium, Launch Everywhere and post-launch featured placements were declined, and no Smol Launch badge was added to BatchCutout.
